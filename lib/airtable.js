...
/*
  Get post need to publish by checking:
    1. Status = Draft and Published Date <= Today
    2. Status = Updated and Published Date <= Today
  Then update Status = 'Published' and 'Published Date' = Today
 */
...
async function getAllMetaPosts() {
  const records = await base('Blog')
    .select({
      fields: ['Slug', 'Metadata', 'Published Date'],
      filterByFormula: `
      AND(
        {Status} = "Published"
      )
